...
and apply the Caratheodory's theorem to establish the existence of equilibrium.
Concretely, suppose $0 \notin Z(\pi, p)$, then 
but $0 \in \text{conv}(Z(\pi, p))$,
by Caratheodory's theorem, there exists $S+1 +1$ vector sucht that 
$\sum \alpha_k z_k = 0 \text{ with } z_k \in Z(\pi, p), \alpha_k \geq 0, \sum \alpha_k = 1$.
Then, we can assign $\alpha_k$ fraction of type $i$ agents to choose $z_{ik}$.
